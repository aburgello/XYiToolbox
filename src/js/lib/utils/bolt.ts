...
export const initBolt = (log = true) => {
  try {
    ...
    if (log) console.log(`initBolt: extRoot="${extRoot}"`);

    if (window.cep && fs.existsSync(jsxSrc)) {
      if (log) console.log("initBolt: loading via fs.existsSync:", jsxSrc);
      evalFile(jsxSrc);
      initializeCEP();
    } else if (window.cep && fs.existsSync(jsxBinSrc)) {
      if (log) console.log("initBolt: loading via fs.existsSync:", jsxBinSrc);
      evalFile(jsxBinSrc);
      initializeCEP();
    } else {
      // Fallback: use ExtendScript's File object to check + load (doesn't
      // depend on window.cep / Node.js require("fs"), which may not be
      // available in some CEP configurations despite CSInterface working).
      if (log) console.log("initBolt: trying ExtendScript fallback load");
      evalES(
        `try{
          (function(){
            var f = new File("${jsxSrc.replace(/\\/g, '/')}");
            if (f.exists) {
              if (typeof $ !== 'undefined' && $.evalFile) {
                $.evalFile(f.fsName);
              }
            } else {
              var fb = new File("${jsxBinSrc.replace(/\\/g, '/')}");
              if (fb.exists && typeof $ !== 'undefined' && $.evalFile) {
                $.evalFile(fb.fsName);
              }
            }
          })();
        }catch(e){
          alert("initBolt error: " + e.toString());
        }`,
        true
      );
      // Give ExtendScript a moment to finish loading before initializing CEP
      initializeCEP();
    }
  } catch (e) {
    console.error("initBolt: initialization error:", e);
  }
};
